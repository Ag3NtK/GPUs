...
// --- VARIABLES EN MEMORIA CONSTANTE (v2) ---
__constant__ float d_sin_table_const[180];
__constant__ float d_cos_table_const[180];

// --- KERNELS BIDIMENSIONALES (v2) ---
...
__global__ void gradient_kernel_2d(float *NR, float *G, float *phi, float *Gx, float *Gy, int height, int width) {
    ...
    if (col >= 2 && col < width - 2 && row >= 2 && row < height - 2) {
        ...
        // CORRECCIÓN 1: Cálculo correcto del ángulo en el rango [0, PI]
        float p = atan2f(Gy[idx], Gx[idx]);
        float PI = 3.141593f;
        if (p < 0.0f) p += PI; 
...
__global__ void hough_kernel_2d_v2(uint8_t *imEdge, uint32_t *accum, int accu_width, int accu_height, int height, int width) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (col < width && row < height) {
        ...
        if (imEdge[idx] > 250) { 
            ...
            for (int theta = 0; theta < 180; theta++) {
                float rho = ((float)col - center_x) * d_cos_table_const[theta] + ((float)row - center_y) * d_sin_table_const[theta];
                int rho_idx = (int)roundf(rho + hough_h);
                
                // CORRECCIÓN 2: Guard de seguridad para evitar Segmentation Fault
...
void lane_assist_GPU(uint8_t *im, int height, int width, int *x1, int *y1, int *x2, int *y2, int *nlines)
{
    ...
    cudaMalloc(&d_imEdge, height * width * sizeof(uint8_t));
    // CORRECCIÓN 3: Inicializar la imagen de bordes a 0 para evitar píxeles basura en los márgenes
    ...
    cudaMemcpyToSymbol(d_sin_table_const, h_sin, 180 * sizeof(float));
    cudaMemcpyToSymbol(d_cos_table_const, h_cos, 180 * sizeof(float));
    ...
    cudaMemset(d_accum, 0, accu_width * accu_height * sizeof(uint32_t));
    hough_kernel_2d_v2<<<grid, block>>>(d_imEdge, d_accum, accu_width, accu_height, height, width);
    ...
    *nlines = 0;
    ...
    printf("=== GPU Timing Results (v2 2D - Constant Mem) ===\n");
    printf("Transferencias H2D:     %.3f ms\n", time_h2d);
    printf("Kernels:                %.3f ms\n", time_kernels);
    printf("Transferencias D2H:     %.3f ms\n", time_d2h);
    printf("Total GPU:              %.3f ms\n", time_h2d + time_kernels + time_d2h);
...
// Dimensiones de bloque y configuración para Shared Memory
...
// =========================================================================
// KERNEL 1: Reducción de Ruido con Shared Memory
// =========================================================================
__global__ void noise_reduction_shmem_kernel(uint8_t *imBW, float *NR, int height, int width) {
    ...
    // Identificador lineal del hilo dentro del bloque
    int tid = ty * blockDim.x + tx;
    int num_threads = blockDim.x * blockDim.y;
    int tile_elements = TILE_SIZE * TILE_SIZE;

    // 1. Carga colaborativa a Shared Memory (incluye el halo/padding)
    for (int i = tid; i < tile_elements; i += num_threads) {
        ...
        if (global_r >= 0 && global_r < height && global_c >= 0 && global_c < width) {
            ...
            tile[tile_r][tile_c] = 0; // Padding
        }
    }

    __syncthreads(); // Sincronización obligatoria antes de leer

    // 2. Cálculo del píxel central de este hilo
    int col = bx + tx;
    int row = by + ty;
    ...
    if (col < width && row < height) {
        ...
        // Se escribe el resultado evitando los bordes totales de la imagen
        if (col >= 2 && col < width - 2 && row >= 2 && row < height - 2) {
...
// =========================================================================
// KERNEL 2: Gradiente de Sobel con Shared Memory
// =========================================================================
__global__ void gradient_shmem_kernel(float *NR, float *G, float *phi, int height, int width) {
    ...
    // 1. Carga colaborativa a Shared Memory
    for (int i = tid; i < tile_elements; i += num_threads) {
        int tile_r = i / TILE_SIZE;
        int tile_c = i % TILE_SIZE;
        int global_r = by + tile_r - FILTER_RADIUS;
...
// =========================================================================
// RESTO DE KERNELS (Supresión, Histéresis y Hough)
// =========================================================================
__global__ void edge_kernel_2d(float *G, float *phi, uint8_t *pedge, int height, int width) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void hough_kernel_2d_const(uint8_t *imEdge, uint32_t *accum, int accu_width, int accu_height, int height, int width) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (col < width && row < height) {
        ...
        if (imEdge[idx] > 250) { 
            ...
            for (int theta = 0; theta < 180; theta++) {
                // Leer desde memoria constante (__constant__)
                float rho = ((float)col - center_x) * d_cos_table_const[theta] + ((float)row - center_y) * d_sin_table_const[theta];
                int rho_idx = (int)roundf(rho + hough_h);
                
                if (rho_idx >= 0 && rho_idx < accu_height) {
                    int acc_idx = rho_idx * accu_width + theta;
                    atomicAdd(&accum[acc_idx], 1); // Operación atómica indispensable
                }
            }
        }
    }
...
// =========================================================================
// HOST FUNCTION: Control general y CPU logic
// =========================================================================
void lane_assist_GPU(uint8_t *im, int height, int width, int *x1, int *y1, int *x2, int *y2, int *nlines)
{
    // 1. Inicializar eventos de tiempo
    cudaEvent_t start_h2d, stop_h2d, start_kernels, stop_kernels, start_d2h, stop_d2h;
    cudaEventCreate(&start_h2d); cudaEventCreate(&stop_h2d);
    cudaEventCreate(&start_kernels); cudaEventCreate(&stop_kernels);
    cudaEventCreate(&start_d2h); cudaEventCreate(&stop_d2h);
    ...
    // 2. Reserva de memoria en el Dispositivo (Device)
    uint8_t *d_imBW, *d_imEdge, *d_pedge;
    ...
    cudaMemset(d_imEdge, 0, height * width * sizeof(uint8_t)); // Evitar ruido residual
    ...
    // 3. Transferencias Host-To-Device
    cudaEventRecord(start_h2d);
    
    // Suponemos que la imagen viene ya preparada o se utiliza el puntero tal cual
    cudaMemcpy(d_imBW, im, height * width * sizeof(uint8_t), cudaMemcpyHostToDevice);
    ...
    // 4. Configuración de ejecución (Grilla 2D)
    dim3 block(BLOCK_SIZE, BLOCK_SIZE);
    dim3 grid((width + block.x - 1) / block.x, (height + block.y - 1) / block.y);

    // 5. Lanzamiento de Kernels
    cudaEventRecord(start_kernels);
    ...
    cudaMemset(d_accum, 0, accu_width * accu_height * sizeof(uint32_t));
    hough_kernel_2d_const<<<grid, block>>>(d_imEdge, d_accum, accu_width, accu_height, height, width);
    
    cudaEventRecord(stop_kernels);
    cudaEventSynchronize(stop_kernels);
    cudaEventElapsedTime(&time_kernels, start_kernels, stop_kernels);

    // 6. Transferencias Device-To-Host
    cudaEventRecord(start_d2h);
    uint32_t *h_accum = (uint32_t *)malloc(accu_width * accu_height * sizeof(uint32_t));
    cudaMemcpy(h_accum, d_accum, accu_width * accu_height * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaEventRecord(stop_d2h);
    ...
    // 7. Extracción de líneas (CPU Logic)
    int threshold = (width > height) ? width / 6 : height / 6;
    *nlines = 0;
    for (int rho = 0; rho < accu_height; rho++) {
        for (int theta = 0; theta < accu_width; theta++) {
    ...
    // 8. Print Results
    printf("=== GPU Timing Results (Shared & Constant Mem) ===\n");
    printf("Transferencias H2D:     %.3f ms\n", time_h2d);
    printf("Kernels:                %.3f ms\n", time_kernels);
    printf("Transferencias D2H:     %.3f ms\n", time_d2h);
    printf("Total GPU:              %.3f ms\n", time_h2d + time_kernels + time_d2h);
...
// =========================================================================
// KERNEL 2: Gradiente de Sobel con Shared Memory (Imágenes)
// =========================================================================
__global__ void gradient_shmem_kernel(float *NR, float *G, float *phi, int height, int width) {
    __shared__ float tile[TILE_SIZE][TILE_SIZE];
...
void lane_assist_GPU(uint8_t *im, int height, int width, int *x1, int *y1, int *x2, int *y2, int *nlines)
{
    ...
    // 3. Configuración y lanzamiento de Kernels
    dim3 block(BLOCK_SIZE, BLOCK_SIZE);
    dim3 grid((width + block.x - 1) / block.x, (height + block.y - 1) / block.y);
