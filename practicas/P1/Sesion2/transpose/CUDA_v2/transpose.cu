...
__global__ void transpose_v2(float *in, float *out, int n) 
{
    // [TILE_DIM][TILE_DIM + 1] para evitar bank conflicts
    ...
    // 1. Cargar tile desde memoria global a shared
    ...
    // 2. Escribir tile transpuesto a memoria global
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS) {
        if (x < n && (y + j) < n) {
...
int main(int argc, char **argv)
{
    ...
    tKernel0 = getMicroSeconds();
    // TODO: launch kernel
    transpose_v2 <<<dimGrid, dimBlock>>> (d_in, d_out, n);
    // TODO: synchronize
    cudaDeviceSynchronize();
    // TODO: measure kernel time
    tKernel1 = getMicroSeconds();
